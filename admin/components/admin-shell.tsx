...
export default function AdminShell({active,children}:{active:string;children:React.ReactNode}){
  ...
  const pageInfo=active==="/subscription"&&typeof window!=="undefined"&&new URLSearchParams(window.location.search).get("tab")==="payments"?["Company & Payments","Legal billing identity, settlement accounts and payment gateway controls"]:active==="/subscription"?["Subscription & Billing","Plans, pricing, entitlements and billing controls"]:active==="/customers"?["Customers","Customer accounts, subscriptions and support"]:active==="/finance"?["Finance & Growth","Revenue, plans, acquisition and commercial performance"]:active==="/organization"?["Organization","Admin users, roles and organizational controls"]:active==="/settings"?["Settings & Platform","Permissions, configuration and platform audit"]:[ "Business Overview","Platform performance, customers and commercial activity" ];
  const allowedLinks=links.filter(([,label,permission])=>permissions.includes(permission)||(roleCode==="SUPER_ADMIN"));
  const currentPath=typeof window!=="undefined"?window.location.pathname+window.location.search:active; const activeAllowed=allowedLinks.some(([href])=>href===currentPath||href===active);
  useEffect(()=>{if(!checking&&!activeAllowed){const fallback=allowedLinks[0]?.[0]||"/login";if(active!==fallback&&currentPath!==fallback)location.replace(fallback)}},[checking,activeAllowed,active,allowedLinks]);
  return checking?<div className="adminLoading"><div className="card">Checking admin access...</div></div>:!activeAllowed?<div className="adminLoading"><div className="card">Checking page permissions...</div></div>:<div className="shell"><aside className="sidebar"><BrandDisplay admin /><nav className="nav">{allowedLinks.map(([href,label])=><a key={href} className={currentPath===href||(href==="/subscription"&&currentPath==="/subscription")?"active":""} href={href}>{label}</a>)}</nav></aside><main className="main"><header className="adminTopbar"><div className="adminPageInfo"><h1>{pageInfo[0]}</h1><span>{pageInfo[1]}</span></div><div className="adminTopbarRight"><span className="lastRefreshed">Last refreshed {lastRefreshed}</span><button className={refreshing?"topRefreshButton refreshing":"topRefreshButton"} onClick={refresh} disabled={refreshing} aria-label="Refresh data">{<RefreshCw size={15} className={refreshing?"refreshSpin":""}/>}<span>{refreshing?"Refreshing...":"Refresh"}</span></button><button className="iconButton" aria-label="Sign out" title="Sign out" onClick={logout}><LogOut size={17}/></button></div></header><div className="adminContent">{children}</div></main></div>;
}
