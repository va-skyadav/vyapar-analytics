...
const MODULES: Array<[string,string]>=[["analytics","Analytics"],["accounting","Accounting"],["inventory","Inventory"],["hr","HR"],["taxation","Taxation"],["predictions","Predictions"],["ai_assistant","AI Assistant"],["advanced_analytics","Advanced Analytics"]];

const blankPlan={code:"",name:"",monthly_price:"0",annual_price:"0",currency_code:"INR",billing_model:"flat_rate",setup_fee:"0",trial_days:"14",minimum_seats:"1",included_seats:"1",overage_enabled:false,overage_config:"{}",is_featured:false,public_visible:true,display_order:"0",is_active:true,description:"",limits:"{}",features:"{}"};
const blankOffer={code:"",name:"",description:"",applicablePlans:[],offer_type:"discount",discount_type:"percent",discount_value:"0",buy_quantity:"2",free_quantity:"1",trial_days:"14",credit_amount:"0",starts_at:"",ends_at:"",max_redemptions:"",max_redemptions_per_customer:"1",first_time_only:false,auto_apply:false,stackable:false,is_active:true};
const blankPayment={code:"",display_name:"",method_type:"bank_transfer",provider:"",is_active:true,is_default:false,beneficiary_name:"",bank_name:"",ifsc_code:"",branch_name:"",upi_id:"",merchant_account_id:"",public_key:"",checkout_url:"",settlement_currency:"INR",instructions:"",account_number:"",gateway_secret:"",webhook_secret:""};
...
export default function Subscription(){
 ...
 const [company,setCompany]=useState<CompanyProfile|null>(null);const [payments,setPayments]=useState<PaymentMethod[]>([]);const [editingPayment,setEditingPayment]=useState<PaymentMethod|null>(null);const [paymentForm,setPaymentForm]=useState<any>(blankPayment);
 ...
 const openPlan=(p:Plan)=>{
  ...
  setPlanForm({code:p.code,name:p.name,monthly_price:String(p.monthly_price??0),annual_price:String(p.annual_price??0),currency_code:(p.currency_code||"INR").trim(),billing_model:p.billing_model||"flat_rate",setup_fee:String(p.setup_fee??0),trial_days:String(p.trial_days??0),minimum_seats:String(p.minimum_seats??1),included_seats:String(p.included_seats??1),overage_enabled:Boolean(p.overage_enabled),overage_config:JSON.stringify(p.overage_config||{},null,2),is_featured:Boolean(p.is_featured),public_visible:p.public_visible!==false,display_order:String(p.display_order??0),is_active:p.is_active,description:p.description||"",limits:JSON.stringify(p.limits||{},null,2),features:JSON.stringify(p.features||{},null,2)});
 ...
 const filteredSubs=subscriptions.filter(s=>(s.businesses?.[0]?.name||"").toLowerCase().includes(search.toLowerCase())||(s.subscription_plans?.[0]?.name||"").toLowerCase().includes(search.toLowerCase())||s.status.toLowerCase().includes(search.toLowerCase()));

 const selectedModules=useMemo<Module[]>(()=>MODULES.map(([code,name])=>planModules[code]||{plan_id:selectedPlanId,module_code:code,module_name:name,enabled:false,limit_value:null,limit_unit:null,notes:null}),[planModules,selectedPlanId]);
 ...
 return <AdminShell active="/subscription">
  ...
  {loading?<div className="card">Loading subscription management...</div>:<>
   ...
   <div className="subscriptionTabs">
    {[["overview","Overview"],["plans","Plans & Pricing"],["offers","Offers & Promotions"],["modules","Modules & Entitlements"],["subscriptions","Customer Subscriptions"],["billing","Billing Rules"],["payments","Company & Payments"]].map(([id,label])=><button key={id} className={tab===id?"subscriptionTab active":"subscriptionTab"} onClick={()=>setTab(id)}>{label}</button>)}
   </div>
   ...
   {tab==="plans"&&<><div className="sectionToolbar"><div><div className="panelTitle">Plans & Pricing</div><div className="muted panelSubtitle">Create and maintain the commercial catalogue.</div></div><button className="primaryButton" onClick={newPlan}>+ New Plan</button></div>
    ...
    {(editingPlan||planForm.name||tab==="plans"&&selectedPlanId==="")&&<section className="card financePanel planEditorPanel"><div className="panelHeader"><div><div className="panelTitle">{editingPlan?"Edit Plan":"Create Plan"}</div><div className="muted panelSubtitle">Define price, packaging, limits and availability.</div></div></div>
     <div className="planIdentityGrid"><label>Plan Code<input className="input" value={planForm.code} onChange={e=>setPlanForm({...planForm,code:e.target.value})}/></label><label>Plan Name<input className="input" value={planForm.name} onChange={e=>setPlanForm({...planForm,name:e.target.value})}/></label><label>Currency<input className="input" maxLength={3} value={planForm.currency_code} onChange={e=>setPlanForm({...planForm,currency_code:e.target.value})}/></label><label>Monthly Price<input className="input" type="number" min="0" value={planForm.monthly_price} onChange={e=>setPlanForm({...planForm,monthly_price:e.target.value})}/></label><label>Annual Price<input className="input" type="number" min="0" value={planForm.annual_price} onChange={e=>setPlanForm({...planForm,annual_price:e.target.value})}/></label><label>Billing Model<select className="input" value={planForm.billing_model} onChange={e=>setPlanForm({...planForm,billing_model:e.target.value})}><option value="flat_rate">Flat rate</option><option value="per_seat">Per seat</option><option value="tiered">Tiered</option><option value="usage_based">Usage based</option><option value="hybrid">Hybrid</option></select></label><label>Setup Fee<input className="input" type="number" min="0" value={planForm.setup_fee} onChange={e=>setPlanForm({...planForm,setup_fee:e.target.value})}/></label><label>Trial Days<input className="input" type="number" min="0" value={planForm.trial_days} onChange={e=>setPlanForm({...planForm,trial_days:e.target.value})}/></label><label>Minimum Seats<input className="input" type="number" min="1" value={planForm.minimum_seats} onChange={e=>setPlanForm({...planForm,minimum_seats:e.target.value})}/></label><label>Included Seats<input className="input" type="number" min="1" value={planForm.included_seats} onChange={e=>setPlanForm({...planForm,included_seats:e.target.value})}/></label><label>Display Order<input className="input" type="number" min="0" value={planForm.display_order} onChange={e=>setPlanForm({...planForm,display_order:e.target.value})}/></label><label className="planDescriptionField">Description<input className="input" value={planForm.description} onChange={e=>setPlanForm({...planForm,description:e.target.value})}/></label></div>
     ...
     <div className="offerChecks"><label><input type="checkbox" checked={planForm.is_featured} onChange={e=>setPlanForm({...planForm,is_featured:e.target.checked})}/> Featured plan</label><label><input type="checkbox" checked={planForm.public_visible} onChange={e=>setPlanForm({...planForm,public_visible:e.target.checked})}/> Visible on pricing page</label><label><input type="checkbox" checked={planForm.overage_enabled} onChange={e=>setPlanForm({...planForm,overage_enabled:e.target.checked})}/> Enable overage billing</label></div><div className="billingSettingsGrid"><label className="billingSettingCard"><strong>Usage Limits</strong><span>Advanced quotas for users, products, transactions, storage and forecasting.</span><textarea className="billingJsonEditor" value={planForm.limits} onChange={e=>setPlanForm({...planForm,limits:e.target.value})}/></label><label className="billingSettingCard"><strong>Feature Entitlements</strong><span>Advanced capabilities that are not represented by module switches.</span><textarea className="billingJsonEditor" value={planForm.features} onChange={e=>setPlanForm({...planForm,features:e.target.value})}/></label><label className="billingSettingCard"><strong>Overage Rules</strong><span>Optional rules for usage beyond included limits.</span><textarea className="billingJsonEditor" value={planForm.overage_config} onChange={e=>setPlanForm({...planForm,overage_config:e.target.value})}/></label></div>
   ...
   {tab==="offers"&&<><div className="sectionToolbar"><div><div className="panelTitle">Offers & Promotions</div><div className="muted panelSubtitle">Manage discount logic, promotional windows and acquisition incentives.</div></div><button className="primaryButton" onClick={newOffer}>+ New Offer</button></div>
    <section className="card financePanel"><div className="tableWrap"><table><thead><tr><th>Offer</th><th>Type</th><th>Rule</th><th>Validity</th><th>Controls</th><th>Status</th><th>Action</th></tr></thead><tbody>{offers.map(o=><tr key={o.id}><td><strong>{o.name}</strong><div className="muted">{o.code}</div></td><td>{o.offer_type.split("_").join(" ")}</td><td>{o.offer_type==="buy_x_get_y"?`Buy ${o.buy_quantity} Get ${o.free_quantity}`:o.discount_type==="percent"?`${o.discount_value}% off`:o.discount_type==="fixed"?`${o.discount_value} fixed`:o.offer_type==="free_trial"?`${o.trial_days} trial days`:"Configured"}</td><td>{o.starts_at?new Date(o.starts_at).toLocaleDateString("en-IN"):"Now"} – {o.ends_at?new Date(o.ends_at).toLocaleDateString("en-IN"):"Open"}</td><td>{o.first_time_only?"First-order · ":""}{o.max_redemptions?o.max_redemptions+" max · ":""}{o.auto_apply?"Auto":"Code"}</td><td><span className={o.is_active?"badge good":"badge"}>{o.is_active?"Live":"Inactive"}</span></td><td><button className="linkButton" onClick={()=>editOffer(o)}>Edit</button></td></tr>)}</tbody></table>{!offers.length&&<div className="empty">No offers configured.</div>}</div></section>
    {offerEditorOpen&&<section className="card financePanel planEditorPanel"><div className="panelHeader"><div><div className="panelTitle">{editingOffer?"Edit Offer":"Create Offer"}</div><div className="muted panelSubtitle">Supports discounts, Buy X Get Y, free trials, credits and price overrides.</div></div></div>
     ...
     <div className="offerPlanSelector"><strong>Applicable Plans</strong><div className="offerPlanChecks">{plans.map(p=><label key={p.id}><input type="checkbox" checked={(offerForm.applicablePlans||[]).includes(p.id)} onChange={e=>setOfferForm({...offerForm,applicablePlans:e.target.checked?[...(offerForm.applicablePlans||[]),p.id]:(offerForm.applicablePlans||[]).filter((x:string)=>x!==p.id)})}/>{p.name}</label>)}</div><small>Leave all unchecked to make the offer eligible across the catalogue.</small></div><div className="offerChecks"><label><input type="checkbox" checked={offerForm.first_time_only} onChange={e=>setOfferForm({...offerForm,first_time_only:e.target.checked})}/> First purchase only</label><label><input type="checkbox" checked={offerForm.auto_apply} onChange={e=>setOfferForm({...offerForm,auto_apply:e.target.checked})}/> Auto apply</label><label><input type="checkbox" checked={offerForm.stackable} onChange={e=>setOfferForm({...offerForm,stackable:e.target.checked})}/> Stackable</label><label><input type="checkbox" checked={offerForm.is_active} onChange={e=>setOfferForm({...offerForm,is_active:e.target.checked})}/> Active</label></div>
     <div className="addAdminActions"><button className="secondaryButton" onClick={()=>{setEditingOffer(null);setOfferEditorOpen(false);setOfferForm({...blankOffer})}}>Close</button><button className="primaryButton" disabled={saving==="offer"} onClick={saveOffer}>{saving==="offer"?"Saving...":editingOffer?"Save Offer":"Create Offer"}</button></div>
    </section>}
   </>}

   {tab==="modules"&&<><div className="sectionToolbar"><div><div className="panelTitle">Modules & Entitlements</div><div className="muted panelSubtitle">Control the packaging layer independently from pricing.</div></div><select className="input modulePlanSelect" value={selectedPlanId} onChange={e=>{const id=e.target.value;setSelectedPlanId(id);const map:Record<string,Module>={};modules.filter(x=>x.plan_id===id).forEach(x=>map[x.module_code]=x);setPlanModules(map)}}><option value="">Select a plan</option>{plans.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
   ...
   {tab==="billing"&&billing&&<><div className="sectionToolbar"><div><div className="panelTitle">Billing Rules</div><div className="muted panelSubtitle">Central policy for trials, renewals, proration, tax and payment recovery.</div></div><button className="primaryButton" disabled={saving==="billing"} onClick={saveBilling}>{saving==="billing"?"Saving...":"Save Billing Rules"}</button></div>
    <section className="card financePanel"><div className="billingRuleGrid">
     ...
     <label>Payment Collection<select className="input" value={billing.payment_collection_mode} onChange={e=>setBilling({...billing,payment_collection_mode:e.target.value as any})}><option value="gateway">Payment Gateway</option><option value="manual">Manual / Bank Transfer</option><option value="hybrid">Gateway + Bank Transfer</option></select></label><label>Default Payment Method<select className="input" value={billing.default_payment_method_id||""} onChange={e=>setBilling({...billing,default_payment_method_id:e.target.value||null})}><option value="">Select method</option>{payments.map(p=><option key={p.id} value={p.id}>{p.display_name}</option>)}</select></label><label>Payment Provider<input className="input" value={billing.payment_provider||""} onChange={e=>setBilling({...billing,payment_provider:e.target.value})}/></label><label>Invoice Prefix<input className="input" value={billing.invoice_prefix} onChange={e=>setBilling({...billing,invoice_prefix:e.target.value.toUpperCase()})}/></label><label>Invoice Terms<input className="input" value={billing.invoice_terms} onChange={e=>setBilling({...billing,invoice_terms:e.target.value})}/></label>
   ...
   {tab==="payments"&&<><div className="sectionToolbar"><div><div className="panelTitle">Company & Payment Collection</div><div className="muted panelSubtitle">Set the legal billing entity and the account or gateway that receives subscription payments.</div></div><button className="primaryButton" disabled={saving==="company"} onClick={saveCompany}>{saving==="company"?"Saving...":"Save Company Details"}</button></div>
    {company&&<section className="card financePanel paymentConfigPanel"><div className="panelHeader"><div><div className="panelTitle">Company Billing Identity</div><div className="muted panelSubtitle">Used on invoices, receipts and customer billing documents.</div></div></div><div className="paymentGrid"><label>Legal Name<input className="input" value={company.legal_name} onChange={e=>setCompany({...company,legal_name:e.target.value})}/></label><label>Trade / Brand Name<input className="input" value={company.trade_name||""} onChange={e=>setCompany({...company,trade_name:e.target.value})}/></label><label>PAN<input className="input" value={company.pan||""} onChange={e=>setCompany({...company,pan:e.target.value.toUpperCase()})}/></label><label>GSTIN<input className="input" value={company.gstin||""} onChange={e=>setCompany({...company,gstin:e.target.value.toUpperCase()})}/></label><label>CIN<input className="input" value={company.cin||""} onChange={e=>setCompany({...company,cin:e.target.value.toUpperCase()})}/></label><label>Billing Email<input className="input" value={company.billing_email||""} onChange={e=>setCompany({...company,billing_email:e.target.value})}/></label><label>Billing Phone<input className="input" value={company.billing_phone||""} onChange={e=>setCompany({...company,billing_phone:e.target.value})}/></label><label>Website<input className="input" value={company.website||""} onChange={e=>setCompany({...company,website:e.target.value})}/></label><label>Address<input className="input" value={company.address_line1||""} onChange={e=>setCompany({...company,address_line1:e.target.value})}/></label><label>City<input className="input" value={company.city||""} onChange={e=>setCompany({...company,city:e.target.value})}/></label><label>State<input className="input" value={company.state||""} onChange={e=>setCompany({...company,state:e.target.value})}/></label><label>Postal Code<input className="input" value={company.postal_code||""} onChange={e=>setCompany({...company,postal_code:e.target.value})}/></label><label>Invoice Prefix<input className="input" value={company.invoice_prefix} onChange={e=>setCompany({...company,invoice_prefix:e.target.value.toUpperCase()})}/></label><label>Invoice Terms<input className="input" value={company.invoice_terms} onChange={e=>setCompany({...company,invoice_terms:e.target.value})}/></label><label>Support Email<input className="input" value={company.support_email||""} onChange={e=>setCompany({...company,support_email:e.target.value})}/></label><label>Support Phone<input className="input" value={company.support_phone||""} onChange={e=>setCompany({...company,support_phone:e.target.value})}/></label></div></section>}
    <section className="card financePanel paymentConfigPanel"><div className="sectionToolbar"><div><div className="panelTitle">Settlement & Payment Methods</div><div className="muted panelSubtitle">Bank account details and gateway secrets are protected with Supabase Vault.</div></div><button className="primaryButton" onClick={newPayment}>+ Add Payment Method</button></div><div className="paymentMethodList">{payments.map(p=><div className="paymentMethodRow" key={p.id}><div><strong>{p.display_name}</strong><span>{p.method_type==="gateway"?(p.provider||"Gateway"):p.method_type==="upi"?"UPI":"Bank Transfer"} · {p.is_active?"Active":"Inactive"}{p.is_default?" · Default":""}</span></div><div><strong>{p.method_type==="bank_transfer"?(p.bank_name||"Bank")+" · ****"+(p.account_number_last4||""):(p.method_type==="upi"?(p.upi_id||"UPI"):(p.provider||"Gateway"))}</strong><span>{p.settlement_currency}</span></div><button className="linkButton" onClick={()=>openPayment(p)}>Edit</button></div>)}{!payments.length&&<div className="empty">No payment method configured.</div>}</div></section>
    <section className="card financePanel paymentConfigPanel"><div className="panelHeader"><div><div className="panelTitle">{editingPayment?"Edit Payment Method":"Add Payment Method"}</div><div className="muted panelSubtitle">Use Bank Transfer for direct settlement, or Gateway for automated card/UPI payments.</div></div></div><div className="paymentGrid"><label>Code<input className="input" value={paymentForm.code} onChange={e=>setPaymentForm({...paymentForm,code:e.target.value})}/></label><label>Display Name<input className="input" value={paymentForm.display_name} onChange={e=>setPaymentForm({...paymentForm,display_name:e.target.value})}/></label><label>Type<select className="input" value={paymentForm.method_type} onChange={e=>setPaymentForm({...paymentForm,method_type:e.target.value})}><option value="bank_transfer">Bank Transfer</option><option value="upi">UPI</option><option value="gateway">Payment Gateway</option></select></label><label>Provider<input className="input" placeholder="Razorpay / Cashfree / Stripe..." value={paymentForm.provider} onChange={e=>setPaymentForm({...paymentForm,provider:e.target.value})}/></label><label>Beneficiary Name<input className="input" value={paymentForm.beneficiary_name} onChange={e=>setPaymentForm({...paymentForm,beneficiary_name:e.target.value})}/></label><label>Bank Name<input className="input" value={paymentForm.bank_name} onChange={e=>setPaymentForm({...paymentForm,bank_name:e.target.value})}/></label><label>Account Number<input className="input" type="password" autoComplete="new-password" placeholder={editingPayment?.account_number_last4?"Configured · re-enter to replace":""} value={paymentForm.account_number} onChange={e=>setPaymentForm({...paymentForm,account_number:e.target.value.replace(/\D/g,"")})}/></label><label>IFSC<input className="input" value={paymentForm.ifsc_code} onChange={e=>setPaymentForm({...paymentForm,ifsc_code:e.target.value.toUpperCase()})}/></label><label>Branch<input className="input" value={paymentForm.branch_name} onChange={e=>setPaymentForm({...paymentForm,branch_name:e.target.value})}/></label><label>UPI ID<input className="input" value={paymentForm.upi_id} onChange={e=>setPaymentForm({...paymentForm,upi_id:e.target.value})}/></label><label>Merchant Account ID<input className="input" value={paymentForm.merchant_account_id} onChange={e=>setPaymentForm({...paymentForm,merchant_account_id:e.target.value})}/></label><label>Public / Publishable Key<input className="input" value={paymentForm.public_key} onChange={e=>setPaymentForm({...paymentForm,public_key:e.target.value})}/></label><label>Checkout URL<input className="input" value={paymentForm.checkout_url} onChange={e=>setPaymentForm({...paymentForm,checkout_url:e.target.value})}/></label><label>Settlement Currency<input className="input" maxLength={3} value={paymentForm.settlement_currency} onChange={e=>setPaymentForm({...paymentForm,settlement_currency:e.target.value.toUpperCase()})}/></label><label>Gateway Secret<input className="input" type="password" autoComplete="new-password" placeholder={editingPayment?.gateway_secret_vault_id?"Configured · re-enter to replace":""} value={paymentForm.gateway_secret} onChange={e=>setPaymentForm({...paymentForm,gateway_secret:e.target.value})}/></label><label>Webhook Secret<input className="input" type="password" autoComplete="new-password" placeholder={editingPayment?.webhook_secret_vault_id?"Configured · re-enter to replace":""} value={paymentForm.webhook_secret} onChange={e=>setPaymentForm({...paymentForm,webhook_secret:e.target.value})}/></label><label className="paymentWide">Customer Payment Instructions<textarea className="billingJsonEditor paymentInstructions" value={paymentForm.instructions} onChange={e=>setPaymentForm({...paymentForm,instructions:e.target.value})}/></label></div><div className="offerChecks"><label><input type="checkbox" checked={paymentForm.is_active} onChange={e=>setPaymentForm({...paymentForm,is_active:e.target.checked})}/> Active</label><label><input type="checkbox" checked={paymentForm.is_default} onChange={e=>setPaymentForm({...paymentForm,is_default:e.target.checked})}/> Default collection method</label></div><div className="addAdminActions"><button className="secondaryButton" onClick={()=>{setEditingPayment(null);setPaymentForm({...blankPayment})}}>Clear</button><button className="primaryButton" disabled={saving==="payment"} onClick={savePayment}>{saving==="payment"?"Saving...":editingPayment?"Save Payment Method":"Add Payment Method"}</button></div></section>
   </>}
  </>}
 </AdminShell>;
}