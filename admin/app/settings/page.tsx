...
export default function Settings(){
 ...
 const uploadBrandAsset=async(key:string,file:File)=>{
  setSaving(key); setError("");
  const allowed=["image/png","image/jpeg","image/webp","image/svg+xml","image/x-icon"];
  if(!allowed.includes(file.type)){setError("Please upload a PNG, JPG, WEBP, SVG or ICO image.");setSaving(null);return}
  if(file.size>5*1024*1024){setError("Image size must be 5 MB or less.");setSaving(null);return}
  const ext=file.name.split(".").pop()?.toLowerCase()||"png";
  const path=key.replace("_url","")+"-"+Date.now()+"."+ext;
  const client=supabase();
  const up=await client.storage.from("brand-assets").upload(path,file,{upsert:false,contentType:file.type,cacheControl:"3600"});
  if(up.error){setError(up.error.message);setSaving(null);return}
  const {data}=client.storage.from("brand-assets").getPublicUrl(path);
  await saveBrandValue(key,data.publicUrl);
  setSaving(null);
 };
