...
async function addSale(formData: FormData) {
  ...
  const { error } = await supabase.rpc("create_sale", {
    ...
    p_unit_price: Number(formData.get("unit_price") || 0),
    p_sale_date: String(formData.get("sale_date") || new Date().toISOString()),
    p_amount_paid: Number(formData.get("amount_paid") || 0)
  });
...
export default async function SalesPage() {
  ...
  return (
    <AppShell businessName={ctx.business.name} title="Sales">
      <section className="content">
        <div className="headerRow">
          <div>
            <h2>Sales</h2>
            <p>Record posted sales so Analytics can calculate real revenue, margin and receivables.</p>
          </div>
        </div>
        <BusinessQuickNav active="sales" />
        <div className="splitGrid">
          <form action={addSale} className="card authForm compactForm">
            <h3>Record sale</h3>
            ...
            <div className="twoCol">
              ...
              <label>Unit price<input name="unit_price" type="number" min="0" step="0.01" defaultValue="0" required /></label>
            </div>
            <div className="twoCol">
              <label>Sale date<input name="sale_date" type="datetime-local" defaultValue={new Date().toISOString().slice(0, 16)} required /></label>
          ...
          <div className="card tableCard">
            <h3>Recent sales</h3>
            {sales?.length ? <div className="tableWrap">
              <table><thead><tr><th>Invoice</th><th>Date</th><th>Total</th><th>Paid</th><th>Status</th></tr></thead>
                <tbody>{sales.map(s => <tr key={s.id}>
                  <td>{s.invoice_number || s.id.slice(0, 8)}</td><td>{new Date(s.sale_date).toLocaleDateString("en-IN")}</td>
                  <td>₹{Number(s.total).toLocaleString("en-IN")}</td><td>₹{Number(s.amount_paid).toLocaleString("en-IN")}</td><td>{s.payment_status}</td>
                </tr>)}</tbody>
              </table>
            </div> : <div className="emptyInline">No sales recorded yet. Analytics will remain zero/empty until real transactions exist.</div>}
          </div>
        </div>
      </section>
    </AppShell>
