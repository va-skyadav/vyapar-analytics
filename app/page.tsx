...
export default async function Dashboard(){
 ...
 const since=new Date(); since.setDate(since.getDate()-30); const sinceIso=since.toISOString();
 const [{data:sales},{data:saleItems},{data:expenses},{data:purchases},{data:purchaseItems},{data:products}]=await Promise.all([
  ...
  supabase.from("sale_items").select("product_id,quantity,total").eq("business_id",ctx.business.id),
 ...
 const costByProduct=new Map<string,number>(); (products||[]).forEach(p=>costByProduct.set(p.id,Number(p.purchase_price||0)));
 const grossProfit=(saleItems||[]).reduce((s,x)=>s+Number(x.total||0)-Number(x.quantity||0)*(costByProduct.get(x.product_id)||0),0);
 const purchasedQty=new Map<string,number>(); (purchaseItems||[]).forEach(x=>purchasedQty.set(x.product_id,(purchasedQty.get(x.product_id)||0)+Number(x.quantity||0)));
 const soldQty=new Map<string,number>(); (saleItems||[]).forEach(x=>soldQty.set(x.product_id,(soldQty.get(x.product_id)||0)+Number(x.quantity||0)));
 const inventoryValue=(products||[]).reduce((s,p)=>{const qty=Number(p.opening_stock||0)+(purchasedQty.get(p.id)||0)-(soldQty.get(p.id)||0); return s+Math.max(qty,0)*Number(p.purchase_price||0)},0);
 ...
 return <AppShell businessName={ctx.business.name} title="Analytics"><section className="content">
  <div className="pageIntro"><div><h2>Business overview</h2><p>See what is happening in your business from real recorded data.</p></div><span className="period">Last 30 days</span></div>
  <div className="grid">{metrics.map(([label,value,caption])=><div className="card metricCard" key={label}><div className="metricLabel">{label}</div><div className="metricValue">{value}</div><div className="metricCaption">{caption}</div></div>)}</div>
  <div className="analyticsGrid">
   <div className="card"><h3>Data status</h3><p className="muted">{(sales?.length||0)+(expenses?.length||0)+(purchases?.length||0)+(products?.length||0)===0?"Your business is ready, but no operating data has been recorded yet.":"These metrics are calculated from your recorded transactions and master data."}</p></div>
